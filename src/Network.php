...
//	use PropertyTrait;
     ...
     * Calculate subnet
     */
	public static function getSubnetByIPv4andMask($ip, $mask) {
		if( empty($ip) || empty($mask) ) return '';
		...
		return long2ip( intval($ip) & intval($mask) ) .'/'. $len;
	}
    ...
    public static function parse($data) {
        if (preg_match('~^(.+?)/(\d+)$~', $data, $matches)) {//10.0.0.0/20
...
//	}
    ...
    public function exclude($exclude) {
        ...
        if (strcmp($exclude->getFirstIP()->inAddr(), $this->getLastIP()->inAddr()) > 0 || strcmp($exclude->getLastIP()->inAddr(), $this->getFirstIP()->inAddr()) < 0
        ) {
    ...
    public function moveTo($prefixLength) {
        ...
        $maxPrefixLength = $this->ip->getMaxPrefixLength(); //32
